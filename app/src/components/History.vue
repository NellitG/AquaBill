      ...
      <div class="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-100">
        ...
        <div class="mb-6 relative">
          ...
          <input
            ...
            class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg custom-focus-ring transition" />
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100 text-sm">
              <tr v-for="(record, index) in paginatedRecords"
                ...
                <td class="py-3 text-gray-600">{{ formatDate(record.date) }}</td>
                <td class="py-3 font-medium text-gray-900">{{ record.client_name }}</td>
                <td class="py-3 text-gray-600">{{ record.meter_number }}</td>
                <td class="py-3 text-gray-600">{{ formatNumber(record.previous_reading) }}</td>
                <td class="py-3 text-gray-600">{{ formatNumber(record.current_reading) }}</td>
                <td class="py-3 text-gray-600 font-medium">{{ formatNumber(record.units_consumed) }}</td>
                <td class="py-3 text-gray-600">Sh {{ formatNumber(record.rate_per_unit) }}</td>
                <td class="py-3 text-blue-600 font-semibold">Sh {{ formatNumber(record.amount) }}</td>

                <td class="py-3 text-gray-600 hover:text-blue-600">
                  <button @click="openReceipt(record)" class="flex items-center space-x-2">
        ...
        <div class="flex justify-center items-center mt-6 space-x-3">
          <button
            ...
            class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md disabled:opacity-50 hover:bg-gray-300">
            ← Previous
          </button>

          <span class="text-sm text-gray-600">Page {{ currentPage }} of {{ Math.max(totalPages, 1) }}</span>
